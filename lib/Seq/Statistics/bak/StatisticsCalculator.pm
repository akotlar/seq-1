...
=cut
package Seq::StatisticsCalculator;

use Moose;
extends 'Seq::StatisticsBase';
...
{ 
  ...
  { 
    ...
    for my $recordHref (@$annotationRecordAref) {
      ...
      if(!index($iupac, $recordAllele) ) { next; }
      if(index($viewedAlleles, $recordAlleleSiteType) ) { next; }
      $viewedAlleles .= $recordAlleleSiteType;

      $self->recordCount($sampleID, $recordAllele, $recordSiteType,
        $recordHref->annotationType);
      $self->recordTrTv($sampleID, $recordAllele, $recordSiteType,
        $recordHref->annotationType);
    }
  }
}
...
sub recordTrTv
{
  my ($self, $sampleID, $sampleAllele, $recordAllele, $recordSiteType, $recordAnnotationType);
