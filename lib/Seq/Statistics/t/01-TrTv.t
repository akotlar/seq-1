...
sub annotate {
  ...
  my $record_href =
    { ref_base => 'G', var_type => 'MULTIALLELIC', genomic_type => 'Exonic' };
  my $gene_data_aref =
    [ 1, 2 ]; #just needs to be longer than 1 to stop workign after tr:tv count
  my $snp_data_aref = [];

  return $self->recordStat(
    $id_genos_href,
    [ $record_href->{var_type}, $record_href->{genomic_type} ],
    $record_href->{ref_base},
    $gene_data_aref, $snp_data_aref
  );
}
...
p $stats;
ok(
  $stats->{P1}{statistics}{'Transitions:Transversions'} == 2,
  'sample has ok Tr:Tv when normal base calls supplied'
);
ok(
  $stats->{P5}{statistics}{'Transitions:Transversions'} == 1,
...
ok( $stats->{P1}{statistics}{'Transitions:Transversions'} == 0,
  'sample ratio ok when no Transitions called' );
...
ok(
  !defined $stats->{statistics}{percentiles},
  'weird bases don\'t generate percentile stats'
);
...
ok(
  $stats->{P1}{statistics}{'Transitions:Transversions'} == $pinf,
...
p $stats;
ok( !defined $stats->{P5}{statistics}{"Transitions:Transversions"},
  "N gives undefined ratio" );
