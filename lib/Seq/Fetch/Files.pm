...
has rsync_bin => ( is => 'ro', isa => 'Str', required => 1, builder => '_build_rsync_bin', );

sub _build_rsync_bin {
  my $self = shift;
  my %rsync_loc_for = ( loc_1 => '/opt/local/bin/rsync',
...
sub fetch_files {
  ...
  for my $file ( @{ $remote_files_aref } ) {
    ...
    my $cmd_href = { srchost => $remote_host,  source => $this_remote_file, dest => $local_dir};
