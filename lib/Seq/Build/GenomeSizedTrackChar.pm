...
sub build_genome_idx {
    ...
    confess
      "build_idx() expected a 3 hashes - exon, flanking exon, and snp sites"
    ...
    for ( my $pos = 0 ; $pos < $self->genome_length ; $pos++ ) {
        ...
        my $site_code =
          $self->get_idx_code( $this_base, $in_gan, $in_gene, $in_exon,
            $in_snp );
...
sub set_gene_regions {
    ...
    confess
"set_gene_regions() requires an array reference of transcript coordinates\n"
