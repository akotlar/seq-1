...
use IO::Socket;
use Sys::Info;
use Sys::Info::Constants qw( :device_cpu ); #for choosing max connections based on available resources
...
use Redis;
  ...
  sub handleJobSuccess
  {
    ...
    try
    {
      my $jsonAnnotationSummary = { 'annotationSummary' => encode_json( $returnJSONreference->{'annotationSummary'} ) };
  ...
  sub handleJobFailure
  {
    ...
    try
    {
      my $jobAttempts = $redis->hincrby($jobKey, 'attempts', 1); #returns int of attempts after increment
  ...
  sub handleJob
  {
    ...
    my $annotationJson;
    my ($inputFile, $outputDir, @submittedJobArray, $jobDetailsHref, $returnJSONreference, $jobAttemptsCount, $userID);
    ...
    try
    {
      ...
      my $log_name = join '.', 'annotation', 'jobID',
        $inputHref->{messageChannelHref}->{recordLocator}->{jobID}, #jobID should be the most atomic, no need to expose userID
        'log';
  ...
  our $W //= !($cpu->count % 2) ? $cpu->count / 2 : !($cpu->count % 3) ? $cpu->count / 3 : $cpu->count || 1; 
  ...
  sub coerceInputs
  {
    my $jobDetailsHref = shift; print Dumper($jobDetailsHref);
...
my $normalQueue = threads->new(sub
  {
    ...
    while (1) 
    {
        my $jobID :shared = $redis->brpoplpush($jobQueueName, $jobsProcessingQueue, 0); #this can result in N identical items in $jobsProcessingQueue; resolved on completion of job on lines 89,116
...
my $failedQueue = threads->new(sub
  {
    my $redis = Redis->new(host => $redisHost, port => $redisPort);

    while (1) 
    {
        my $jobID :shared = $redis->brpoplpush($jobsFailedQueueName, $jobsProcessingQueue, 0); #this can result in N identical items in $jobsProcessingQueue; resolved on completion of job on lines 89,116
